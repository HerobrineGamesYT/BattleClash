...
public class Archer extends Class {
	...
	public void onLaunch(ProjectileLaunchEvent e) {
		...
		if (e.getEntity() instanceof Arrow) {
			...
			// If a player fully charged their bow, we won't worry about its range!
			// This is an experimental change.
			// We also now allow the arrow to have infinite range if they aren't playing Towers mode.
			if (arrow.isCritical()) return;
			if (!Manager.getArena(arrow.getLocation().getWorld()).getType().equals(GameType.CLASH_ROYALE)) return;
				...
				if (player.getUniqueId().equals(this.getUUID())) {
					...
					new BukkitRunnable() {
						...
						public void run() {
							...
							if (arrowLocations.get(0).distance(arrow.getLocation()) > ClassTypes.ARCHER.getProjectileRange()
