...
public class Witch extends Class {
	...
	public void setSkeletonTarget(Skeleton skeleton, Player target) {
						...
						for (UUID uuid : arena.getPlayers()) {
							...
							if (i == arena.getPlayers().size()) {
								...
								if (playerTeam != player2Team) {
									...
									skeletonTargets.put(player2.getUniqueId(), player2.getLocation());
								}
							}
						...
						for (UUID uuid : skeletonTargets.keySet()) {
							...
							if (skeletonTargets.containsKey(uuid)) {
								...
								if (uuid != player.getUniqueId()) {
									...
									if (arena.getTeam(player3) != arena.getTeam(player)) {
										skeletonTargetDistance.put(player3.getUniqueId(),
												player.getLocation().distanceSquared(player3.getLocation()));
													...
													for (String b : plugin.getConfig().getConfigurationSection("towers")
														...
														if (!b.equals("world") && !b.equals("arena")) {
															...
															if (region.blockInLocation(block)) {
																player.sendMessage(
																		Colorize.color("&2&lDEBUG!&r&a You have hit "
																				+ tower.getName() + " !"));
																...
																if (tower.isEnabled() && !tower.getTeam()
																	...
																	if (game.getTowerHits()
																		...
																		if (System.currentTimeMillis()
																			...
																			game.updateTowerHealth(Manager.getArena(
																					tower.getRegionLocations()[1]
																							.getWorld()));
																			if (tower.getHealth() <= 0) {
																				...
																				for (Cannon cannon : tower
																						.getCannons()) {
																					cannon.setActive(false);
																				}
																				if (tower.getType()
																						.equals(TowerTypes.PRINCESS)) {

																					for (Tower tower2 : game.getTower()
																							.values()) {
																						if (tower2.getTeam()
																								.equals(tower.getTeam())
																								&& tower2.getType()
																										.equals(TowerTypes.KING)) {
																							if (!tower2.isEnabled()) {
																								tower2.setEnabled(true);
																								for (Cannon cannon : tower2
																										.getCannons()) {
																									cannon.setActive(
																											true);
																								}
											...
											if (ent instanceof Player) {
												...
												if (arena.getTeam(player) != arena.getTeam(targetPlayer)) {
													...
													if (!hasHit.containsKey(player.getUniqueId())) {
														...
														GameListener.customDeathCause.put(targetPlayer.getUniqueId(),
																CustomDeathCause.WITCH_MAGIC);
														// commented out messages as they are spammed in chat even
														// though player is only damaged once //
														// player.sendMessage(ChatColor.LIGHT_PURPLE + "You hit "
														// + arena.getTeam(targetPlayer).getColor()
														// + targetPlayer.getName() + ChatColor.LIGHT_PURPLE
														// + " with your magic ability!");
														// targetPlayer.sendMessage(arena.getTeam(player).getColor()
														// + player.getName() + ChatColor.LIGHT_PURPLE
														// + " hit you with their magic ability!");
													} else {
														return;
													}
												...
												if (arena.getTeam(player) != arena.getTeam(skeletonOwner)) {
													...
													// + skeletonOwner.getName() + "'s " + ChatColor.LIGHT_PURPLE
													// + "skeleton with your magic ability!");
												}
